...
def pull_image():  # Method for pulling/downloading the docker image form the docker hub
    ...
    image = input(
        "Enter the image name[also specify the version if you want]-:")
    o = subprocess.run("docker pull {}".format(image), shell=True)
    if o.returncode == 0:
...
def launch_container():  # Method for launching the docker container
    ...
    if len(os_name) != 0:
        subprocess.run(
            "docker run -it --name {} {}".format(os_name, image_name), shell=True)
    else:
...
def remove_container():  # method for removing the container
    ...
    image_or_id_of_os = input(
        "Enter the image or id of the container/os you want to delete-:")
...
def see_logs():  # Method for seeing the logs of a container
    ...
    image_or_id_of_os = input(
        "Enter the name or id of the conatiner/os whose logs you want to see-:")
...
def cp_base_to_cont():  # Method for copying the content from the base os to the conatiner
    ...
    content = input(
        "Enter the path of the content you want to copy from os to container-:")
    os = input("Enter the os name or id-:")
    path_in_os = input(
        "Enter the location in the conatiner where you want to store the content-:")
    o = subprocess.run("docker cp {} {}:{}".format(
        content, os, path_in_os), shell=True)
    if o.returncode == 0:
...
def cp_cont_to_base():  # Method for copying the content  from container to the base os
    ...
    o = subprocess.run("docker cp {}:{}  {}".format(
        os, content, path_in_base), shell=True)
    if o.returncode == 0:
        print("The file conetent has been copied to the respective docker conatiner successfully..:)")
    else:
        print("Some error happened..:(")
